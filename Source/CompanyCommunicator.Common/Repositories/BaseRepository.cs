...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Repositories
{
        ...
        where T : TableEntity, new()
    {
        ...
        /// <summary>
        ///  Get all sentNotificationDataEntity with given MessageID. Used to put reactions count.
        /// </summary>
        /// <param name="messageId">Message ID.</param>
        /// <returns>A <see cref="Task{TResult}"/> representing the result of the asynchronous operation.</returns>
        public async Task<T> GetAllEqualToMessageIDAsync(string messageId)
        {
            var filterByMessageId = TableQuery.GenerateFilterCondition("MessageId", QueryComparisons.Equal, messageId);
            System.Diagnostics.Trace.TraceError("check, Message ID " + messageId);
            this.Logger.LogInformation("check, Message ID " + messageId);
            var query = new TableQuery<T>().Where(filterByMessageId);
            System.Diagnostics.Trace.TraceError("check, query " + query);
            this.Logger.LogInformation("check, query " + query);
            var entities = await this.ExecuteQueryAsync(query);

            System.Diagnostics.Trace.TraceError("entities Count: " + entities.Count.ToString());
            this.Logger.LogInformation("entities Count: " + entities.Count.ToString());
            System.Diagnostics.Trace.TraceError("SENT NOTIFICATION PARTItION kEY: " + entities.First().PartitionKey);
            this.Logger.LogInformation("SENT NOTIFICATION PARTItION kEY: " + entities.First().PartitionKey);
