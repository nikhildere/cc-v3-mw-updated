...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Controllers
{
    ...
    public class SentNotificationsController : ControllerBase
    {
        ...
        /// <param name="reactionDataRepository">Reaction Data Repository required to get reactions Count.</param>
        ...
        public async Task<IEnumerable<SentNotificationSummary>> GetSentNotificationsAsync()
        {
            ...
            foreach (var notificationEntity in notificationEntities)
            {
                var summary = new SentNotificationSummary
                {
                    ...
                    Clicks = notificationEntity.Clicks,
                    ReactionsCount = notificationEntity.ReactionsCount,
        ...
        public async Task<IActionResult> GetSentNotificationByIdAsync(string id)
        {
            ...
            // Added:Reaction Count - logic from DataStreamFacade.cs, hve put logic on BlobAdded
            /*var sentNotificationDataEntitiesStream = this.sentNotificationDataRepository.GetStreamsAsync(id);
            await foreach (var sentNotificationDataEntities in sentNotificationDataEntitiesStream)
            {
                notificationEntity.ReactionsCount = 0;
