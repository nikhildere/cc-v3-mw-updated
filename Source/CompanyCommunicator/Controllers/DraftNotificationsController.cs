...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Controllers
{
    ...
    public class DraftNotificationsController : ControllerBase
    {
        ...
        // Added: isImageUploadBlobStorage variable to pass to CreateDraftNotificationAsync method
        private bool isImageUploadBlobStorage = false;
            ...
            IOptions<UserAppOptions> userAppOptions)
        {
            ...
            if (this.userAppOptions.ImageUploadBlobStorage)
            {
                this.isImageUploadBlobStorage = true;
            }
        }
        ...
        public async Task<ActionResult<string>> CreateDraftNotificationAsync([FromBody] DraftNotification notification)
        {
            ...
            // Added: Tracking URL col in the draft notifications table
            notification.TrackingUrl = this.HttpContext.Request.Scheme + "://" + this.HttpContext.Request.Host + "/api/sentNotifications/tracking";

            if (notification.MessageType != "Poll")
            {
                notification.ClickRateUrl = this.HttpContext.Request.Scheme + "://" + this.HttpContext.Request.Host + "/redirect";
            }

            var notificationId = await this.notificationDataRepository.CreateDraftNotificationAsync(
                notification,
                this.isImageUploadBlobStorage,
        ...
        public async Task<IActionResult> UpdateDraftNotificationAsync([FromBody] DraftNotification notification)
        {
            ...
            var notificationEntity = new NotificationDataEntity
            {
                ...
                ClickRateUrl = this.HttpContext.Request.Scheme + "://" + this.HttpContext.Request.Host + "/redirect",
            ...
            if (this.isImageUploadBlobStorage && !string.IsNullOrEmpty(notification.ImageLink) && notification.ImageLink.StartsWith(Constants.ImageBase64Format))
            {
        ...
        public async Task<ActionResult<DraftNotification>> GetDraftNotificationByIdAsync(string id)
        {
            ...
            var result = new DraftNotification
            {
                ...
                ClickRateUrl = notificationEntity.ClickRateUrl,
